...
- Break of structure is the ONLY mandatory entry trigger: the close must clear the nearest swing high/low (pivots with entry.swingStrength bars each side within entry.structureLookbackMin minutes, falling back to the session's extreme high/low so far if no pivot has confirmed yet) by more than entry.breakBufferPoints.
- Two secondary confluences upgrade (never gate) the setup grade to A/A+: a "displacement" candle — true range at least entry.displacementSizeRatio × the ~10-bar average true range AND at least entry.displacementPrevRatio × the previous bar's true range, with wick fraction ≤ entry.maxWickRatio — and HTF bias, approximated by aggregating the most recent entry.htfBarMinutes 1-min bars into a synthetic candle and checking it agrees with the trade direction. Grade is A+ if both align, A if one aligns, B+ if neither (BOS alone) — all grades are tradeable.
